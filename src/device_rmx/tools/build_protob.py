...
RMX_PROTOBUF = os.path.join(ROOT_DIR, "src", "device_rmx", "protobuf")
RMX_MESSAGES = os.path.join(CWD, "messages")
...
#        selected += ["messages-debug.proto"]
    ...
    proto_srcs = [os.path.join(RMX_PROTOBUF,  x) for x in selected]
    exec_args = [
        sys.executable,
